...
class CourseViewController: UIViewController, TitleManager{
    
    private var currentCategory = "All"
    func didUpdateTitle(with title: String) {
        
        currentCategory = title // sets the current button title
        courseTableView.reloadData() // reloads the tableview to reflect the appropriate data to be found in index 2
    }
    ...
    private let courseTableView: UITableView = {
        ...
        table.register(TrendingCoursesCollectionViewTableViewCell.self, forCellReuseIdentifier: TrendingCoursesCollectionViewTableViewCell.identifier)
        
        table.register(DesignCoursesCollectionViewTableViewCell.self, forCellReuseIdentifier: DesignCoursesCollectionViewTableViewCell.identifier)
        
        table.register(DevelopmentCoursesCollectionViewTableViewCell.self, forCellReuseIdentifier: DevelopmentCoursesCollectionViewTableViewCell.identifier)
    ...
    override func viewDidLoad() {
        ...
        navigationController?.navigationItem.largeTitleDisplayMode = .always // the 8 lines of code above don't yet affect the font style of the title. Find a better solution 
...
extension CourseViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 1:
            ...
            //currentCategory = cell.getActiveButtonTitle()
            cell.delegate = self
            return cell
        case 2:
            if currentCategory == "All"{
                ...
                guard let cell = tableView.dequeueReusableCell(withIdentifier: TrendingCoursesCollectionViewTableViewCell.identifier, for: indexPath) as? TrendingCoursesCollectionViewTableViewCell else{return UITableViewCell()}
                return cell}
            else if currentCategory == "Design"{
                guard let cell = tableView.dequeueReusableCell(withIdentifier: DesignCoursesCollectionViewTableViewCell.identifier, for: indexPath) as? DesignCoursesCollectionViewTableViewCell else{return UITableViewCell()}
                return cell}
            else {
                guard let cell = tableView.dequeueReusableCell(withIdentifier: DevelopmentCoursesCollectionViewTableViewCell.identifier, for: indexPath) as? DevelopmentCoursesCollectionViewTableViewCell else{return UITableViewCell()}
                return cell}
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        case 1:
            return "Categories"
        case 2:
            return currentCategory //"All Courses" // the only section title that mutates
        default:
            return nil
        }
